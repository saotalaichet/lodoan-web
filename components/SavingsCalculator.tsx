...
export default function SavingsCalculator({ lang }: Props) {
  ...
  function adjOrders(d: number) {
    setOrders(o => Math.max(1, Math.min(500, o + d)));
  }
  ...
  function onBlur() {
    ...
    const clamped = Math.max(10000, Math.min(5000000, parsed));
    setRawValue(clamped);
  ...
  function onValueChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (parsed >= 10000) setRawValue(Math.min(5000000, parsed));
  }
              ...
              <div style={stepperStyle}>
                ...
                <div style={valWrap}>
                  <input
                    ...
                    value={orders}
                    min={1} max={500}
                    onChange={e => setOrders(Math.max(1, Math.min(500, parseInt(e.target.value) || 1)))}
                    style={inputStyle}
                  />
                </div>
                <button style={btnStyle} onClick={() => adjOrders(5)}>+</button>

...
export default function OvenlyAboutPage() {
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', background: BG, color: '#1a1a1a', lineHeight: 1.6 }}>
      <style>{`
        .ov-hero { padding: 80px 40px 72px; text-align: center; }
        .ov-section-pad { padding: 72px 40px; }
        .ov-problem-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 80px; align-items: center; }
        .ov-values-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 24px; }
        .ov-founder-pad { padding: 72px 40px; text-align: center; }
        .ov-cta-pad { padding: 72px 40px; text-align: center; }
        .ov-footer-pad { padding: 40px; text-align: center; }
        .ov-stats-box { background: ${BG}; border-radius: 20px; padding: 48px 40px; border: 1px solid ${BORDER}; }
        @media (max-width: 768px) {
          .ov-hero { padding: 48px 20px 40px; }
          .ov-section-pad { padding: 48px 20px; }
          .ov-problem-grid { grid-template-columns: 1fr; gap: 32px; }
          .ov-values-grid { grid-template-columns: 1fr; }
          .ov-founder-pad { padding: 48px 20px; }
          .ov-cta-pad { padding: 48px 20px; }
          .ov-footer-pad { padding: 32px 20px; }
          .ov-stats-box { padding: 32px 24px; }
        }
      `}</style>
      ...
      <section className="ov-hero">
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h1 style={{ fontSize: 'clamp(28px, 5vw, 64px)', fontWeight: 800, letterSpacing: '-2px', lineHeight: 1.06, marginBottom: 24, maxWidth: 820, margin: '0 auto 24px' }}>
            {isVI ? 'Chúng tôi giúp quán ăn Việt Nam phát triển' : 'We help Vietnamese restaurants thrive'}
          </h1>
          <p style={{ fontSize: 'clamp(15px, 2vw, 18px)', color: '#555', lineHeight: 1.8, maxWidth: 600, margin: '0 auto' }}>
            {isVI
      ...
      <section style={{ background: '#fff', borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div className="ov-section-pad" style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="ov-problem-grid">
            <div>
              <p style={{ fontSize: 13, fontWeight: 700, color: PRIMARY, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 20 }}>
                {isVI ? 'Vấn đề' : 'The problem'}
              </p>
              <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 38px)', fontWeight: 800, letterSpacing: '-1px', lineHeight: 1.15, marginBottom: 20 }}>
            ...
            <div className="ov-stats-box">
              <p style={{ fontSize: 'clamp(36px, 5vw, 48px)', fontWeight: 800, color: PRIMARY, margin: '0 0 8px', letterSpacing: '-2px' }}>25-30%</p>
              ...
              <p style={{ fontSize: 'clamp(36px, 5vw, 48px)', fontWeight: 800, color: '#111', margin: '0 0 8px', letterSpacing: '-2px' }}>100%</p>
      ...
      <section>
        <div className="ov-section-pad" style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 38px)', fontWeight: 800, letterSpacing: '-1px', marginBottom: 40, maxWidth: 500 }}>
            {isVI ? 'Chủ quán xứng đáng được đối xử tốt hơn' : 'Restaurant owners deserve better'}
          </h2>
          <div className="ov-values-grid">
            {[
              { title: isVI ? 'Chủ quán làm chủ' : 'Owners own their channel', text: isVI ? 'Kênh bán hàng của bạn là của bạn. Không ai có quyền lấy đi khách hàng mà bạn đã xây dựng.' : 'Your sales channel belongs to you. No one should be able to take away the customers you have built.' },
              { title: isVI ? 'Công nghệ dễ dùng' : 'Technology made simple', text: isVI ? 'Công nghệ tốt nhất là công nghệ bạn không cần phải lo lắng. Chúng tôi lo phần kỹ thuật để bạn tập trung vào nấu ăn.' : 'The best technology is the kind you never worry about. We handle the tech so you can focus on cooking.' },
              { title: isVI ? 'Minh bạch' : 'Transparent', text: isVI ? 'Không có phí ẩn. Không có điều khoản phức tạp. Bạn biết chính xác bạn đang trả gì và nhận lại được gì.' : 'No hidden fees. No complicated terms. You know exactly what you are paying and what you get in return.' },
              { title: isVI ? 'Cùng phát triển' : 'We grow together', text: isVI ? 'Thành công của chúng tôi phụ thuộc vào thành công của bạn. Chúng tôi chỉ thực sự phát triển khi quán của bạn phát triển.' : 'Our success depends on your success. We only truly grow when your restaurant grows.' },
            ].map((v, i) => (
      ...
      {/* Founder quote */}
      <section style={{ background: '#fff', borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div className="ov-founder-pad" style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ fontSize: 'clamp(16px, 2.5vw, 24px)', fontWeight: 700, color: '#111', lineHeight: 1.5, marginBottom: 32, maxWidth: 700, margin: '0 auto 32px' }}>
            {isVI
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#FDF0EE', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <section style={{ background: PRIMARY }}>
        <div className="ov-cta-pad" style={{ maxWidth: 900, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px, 4vw, 40px)', fontWeight: 800, color: '#fff', marginBottom: 16, letterSpacing: '-1px' }}>

...
const T = {
  vi: {
    title: 'Liên Hệ', fullName: 'Họ và Tên', restaurantName: 'Tên nhà hàng (nếu có)',
    phone: 'Số Điện Thoại', email: 'Email', city: 'Thành Phố', message: 'Tin Nhắn',
    send: 'Gửi', sending: 'Đang gửi...', success: 'Cảm ơn bạn! Chúng tôi sẽ liên hệ sớm.',
    sendAnother: 'Gửi thêm',
  },
  en: {
    title: 'Contact Us', fullName: 'Full Name', restaurantName: 'Restaurant name (optional)',
    phone: 'Phone Number', email: 'Email', city: 'City', message: 'Message',
    send: 'Send', sending: 'Sending...', success: 'Thank you! We will be in touch soon.',
  ...
  return (
    <div className="min-h-screen" style={{ background: 'hsl(30,20%,97%)' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <h1 className="font-heading text-3xl font-black mb-6" style={{ color: PRIMARY }}>{t.title}</h1>

        {success ? (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-8 text-center">
            <span className="text-4xl">✅</span>
            <p className="text-green-700 font-semibold text-lg mt-4">{t.success}</p>
            <button onClick={() => setSuccess(false)} className="mt-4 text-sm font-semibold hover:underline" style={{ color: PRIMARY }}>
              {t.sendAnother}
            </button>
          </div>
        ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea required value={form.message} onChange={e => update('message', e.target.value)}
                  rows={4} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:border-red-800 resize-none transition-all" />
              </div>

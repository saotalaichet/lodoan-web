...
function MapView({ lat, lng, name }: { lat: number; lng: number; name: string }) {
  ...
  useEffect(() => {
    ...
    el.style.cssText = `width:36px;height:36px;background:var(--color-primary,#8B1A1A);border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.3);`;
...
function StatusBar({ status, orderType, lang }: { status: string; orderType: string; lang: string }) {
  ...
  const currentIdx = status === 'cancelled' || status === 'timed_out'
    ? -1 : steps.indexOf(status) === -1 ? 0 : steps.indexOf(status);
          ...
          <div key={step} style={{ display: 'flex', alignItems: 'center', flex: idx < steps.length - 1 ? 1 : 0 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                ...
                boxShadow: active ? '0 0 0 4px rgba(139,26,26,0.2)' : 'none',
    ...
    try {
      ...
      ws.onmessage = (e) => {
        try {
          ...
          if (msg.type === 'status_update' && msg.order) setOrder(msg.order);
        } catch {}
      };
    ...
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', background: '#FAF8F0', minHeight: '100vh', color: '#1a1a1a' }}>
      <style>{`
        ...
        @media (min-width: 768px) {
          ...
          .mobile-map-wrap { display: none !important; }
        }
        @media (max-width: 767px) {
          .track-sheet { padding: 16px 16px 80px !important; }
        }
      `}</style>
      ...
      <div className="track-layout" style={{ display: 'block' }}>
        ...
        {/* Map — desktop left column only */}
        <div className="track-map" style={{ display: 'none', background: '#F5F0EA', position: 'relative' }}>
          {hasMap ? (
        ...
        <div className="track-sheet" style={{ padding: '20px 20px 40px', background: '#FAF8F0' }}>
          ...
          {/* Map — mobile only */}
          {hasMap && (
            <div className="mobile-map-wrap" style={{ height: 220, borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
              <MapView lat={Number(restaurant.latitude)} lng={Number(restaurant.longitude)} name={restaurant.name || order.restaurant_name} />
            </div>
          )}
